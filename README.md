...
│   ├── chat/        核心对话页（⭐我的常用语(已存N/5提示·满5条替换最早一条·删除二次确认·按使用次数自动置前·显示次数与最近使用时间)/夜间柔和模式(22:00-6:00 自动·进入当天轻提一句护眼·🌙手动开关优先)/快捷情绪双闭环/此刻强度滑条(7种情绪标签含🌟期待)/空输入点发送温柔托底(换引导占位)/输入超150字显示字数/倾诉后小结算/AI头像情绪态/打字草稿跨会话保留(未发送内容切走不丢)/深夜工具箱·写给自己+5分钟急救(固定预案)+睡前托一句话明早回读/📋一键复制整段对话留档(带时间戳与主题行)/重新开始前可先备份一份/回复引用/长按AI回复「换个说法」重新生成/长按任意消息存进想法盒(7天后回看)/一键总结这轮对话(总结后引导顺手去心情页记一笔)/长对话折叠·一键收起·消息跨天日期分隔线(今天/昨天/M月D日)/连续低落情绪关怀条(可顺手写给此刻的自己)/周一晨间上周陪伴小结(条数·天数·主要情绪·每周一次)/高危信号首次强调认真对待/自我安抚快捷短句/夸夸我本地词库+和我做3次深呼吸即时引导/撤回我的消息/输入框按时段共鸣占位/英文角/天气/今日心语/晨间轻提醒/计划日提示/重新开始对话/长按珍藏·含撤回/今日第  N 次倾诉徽章/陪伴身份(朋友/树洞/清醒教练·本地记住)/陪伴里程碑(111·333·999句)/底栏已聊几 句·写字数计数）
│   ├── mood/      情绪可视化（本周小结一键复制/快速记录(强度1-5可选·toast带本周第N条实感)/记录里程碑庆祝(第10/50/100/200条一次性)/8种心情/健康指数/14天洞察(色带14/30天⇄切换·8情绪图例·今天描边高亮·空白天虚线区分·🏆最长连记天数)·连低连高解读·曲线今日锚点/色带点天小结+写一句补记+长按管理/日历(周末暖底色)+翻月回本月·当月最长连续记录段+当月最强情绪日提示/📋一键复制月报(条数/天数/Top3/晚间小结篇数)·本月触发来源排行·环比洞察(本月vs上月Top1)·记录时段偏好(最常何时记)·近30天强度分布(1-5各档占比)·本月情绪转换统计(最常见A→B转变链)/今日未记录轻提醒/未打卡可一键回顶部记录/健康指数❓说明(含较上周对比·修复说明文案不显示bug)·本月记录天数徽章·情绪断崖预警/三件小事(💡灵感一键填入·四场景/勾选记完成时刻·连续全完成天数🔥)·全完成庆祝/记录后暖心回应+负性情绪「情绪急救包」(3件即时小事·一键去呼吸/充电站)·分布一句话解读/最像此刻的一天回看/时光信）
...
│   ├── breathe/   呼吸放松练习（4 轮引导/4 档节奏 4-4-6·4-4-4-4·6-2-6-2+发呆2分钟(累计次数陪伴计数)+自定义节奏(吸/屏/呼 2-10 秒可调·一键填入4-7-8助眠/4-4-6/6-2-6)+按时段推荐节奏(晨盒式/午放松/夜长呼)/练习中断24h内温柔回访提示·结束可选 1-5 强度记录平静·toast带本周第N次(发呆模式自动隐藏强度·文案专属)/呼吸徽章/累计+本周进度统计·每日 10 分钟目标进度条+达标连击+近4周放松热力图+完成时段分布(早/中/晚/深夜)+本周vs上周次数对比·🏅最长连练纪录+最近7次放松回看·完成练习「呼吸回响」(发呆模式结束自动给专属回响·→直达心理小课为什么呼吸有用)，全程本地）
