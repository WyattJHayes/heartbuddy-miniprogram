...
│   ├── helper/    求助（热线(长按卡片复制号码·🕘最近拨打记录鼓励性回显(带相对时间)·🫁开口前先做60秒呼吸直达)/安全包·我提前信任的人名单(支持存电话·危急一键拨打·完整度提示(N人M电话)·30天未更新保鲜提醒·长按清号码·📤导出/📥剪贴板恢复)/1h·24h·3d·7d四段温暖回访/走完四段一次性「毕业卡」致谢/回访关怀卡/求助后静音关怀(可提前手动结束)/智能分流提示/「现在最需要什么」三方向快速分流/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/开口求助话术生成器(9种处境·默认显示4个可展开全部→生成第一句话·可复制/✏️改一改再复制/换说法·最近复制过可再复制·含被欺负/家里吵架)+考试季家长贴士一键复制转发/🫂「帮助朋友」危机卡(怎么陪·怎么问安全·一键复制)/一键复制求助文字发信任的人/今日关怀/英语角）
...
│   ├── station/   情绪充电站（按此刻心情筛选(焦虑/难过/生气/睡前/学习/平静)·12 张可展开心理急救卡(展开自动记阅读次数·📖最常翻Top1提示)(含拖延启动式·睡前两行清单·发呆 2 分钟·气得发抖时 60 秒降火·5-4-3-2-1接地术·考前一夜3件事·情绪词库准确命名)/一键跳呼吸·聊天·记下此刻心情·「🎴随机抽一张」(第N次抽中缘分提示)·卡片长按★收藏置顶(最多3张·收藏概览提示)·展开可📄复制单张·入口「心理小课」/可分享·今日一句话(换日轮换·每日可手动换一次)）
...
│   └── profile/   我的（数据足迹含年度统计(今年N条·覆盖N天)+心理小课进度条直达/成就徽章×16(🔥连学7天·已点亮自动置前并记录点亮日期·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课7节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·覆盖全部30+本地键·云端心情与徽章保留)/复制文字版「我的概览」(含结业与温故题数)+「数据都放在哪」透明说明/数据足迹(近7天格子带星期标签·本月记录天数进度条)+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候+累计陪伴天数(7/30/100/365天里程碑文案·可复制留念)·最近自评直达/好久没回温柔卡）
