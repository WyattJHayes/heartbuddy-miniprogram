...
│   ├── mood/      情绪可视化（本周小结一键复制/快速记录(强度1-5可选·toast带本周第N条实感)/记录里程碑庆祝(第10/50/100/200条一次性)/8种心情/健康指数/14天洞察(色带14/30天⇄切换·8情绪图例·空白天虚线区分·🏆最长连记天数)·连低连高解读·曲线今日锚点/色带点天小结+写一句补记+长按管理/日历+翻月回本月·当月最强情绪日提示/📋一键复制月报(条数/天数/Top3/晚间小结篇数)·本月触发来源排行·本月情绪转换统计(最常见A→B转变链)/今日未记录轻提醒/未打卡可一键回顶部记录/健康指数❓说明(含较上周对比·修复说明文案不显示bug)·本月记录天数徽章·情绪断崖预警/三件小事(💡灵感一键填入·四场景/勾选记完成时刻)·全完成庆祝/记录后暖心回应+负性情绪「情绪急救包」(3件即时小事·一键去呼吸/充电站)·分布一句话解读/最像此刻的一天回看/时光信）
...
│   ├── scan/      5 分钟身体扫描（按时段扫描建议·10 段引导计时·完成自动记「平静」+可选 1-5 强度+身体感受一词(松了/暖了…留最近5次回看)/累计完成次数/再来一次/每日一句轮换开场）
...
│   ├── edu/       心理小课（课程带第N节序号/🔍课程内容搜索(无结果友好提示)/7 节心理健康小课程·本地：认识情绪/考前焦虑/睡不着/低落最小行动/朋友闹掰/社媒比较心/向大人求助不丢人·打开自动定位第一节未学(续学)·每课📄一键复制分享(带学习进度)·随堂一问答对自动点亮并自动展开下一节·完成度进度条·🔥连续学习天数·✏️学习心得一句话(本地·可复制)·学完7节出结业卡(记结业日期·结业词可复制)·每日温故一题(答错进错题本·优先抽错题·结业卡展示累计答对题数)·一键重置·热线直达）
