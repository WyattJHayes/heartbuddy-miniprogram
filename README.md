...
├── utils/api.js                      # 云函数封装
├── pages/
│   ├── welcome/   隐私 & AI 说明 + 首次 3 步新手引导卡(可点直达对应页·可跳过) + 🆕最近更新日志(可折叠)
...
│   ├── mood/      情绪可视化（本周小结一键复制/快速记录(强度1-5可选·toast带本周第N条实感)/记录里程碑庆祝(第10/50/100/200条一次性)/8种心情/健康指数/14天洞察(色带14/30天⇄切换·8情绪图例·今天描边高亮·空白天虚线区分·🏆最长连记天数)·连低连高解读·曲线今日锚点/色带点天小结+写一句补记+长按管理/日历(周末暖底色)+翻月回本月·当月最长连续记录段+当月最强情绪日提示/📋一键复制月报(条数/天数/Top3/晚间小结篇数)·本月触发来源排行·环比洞察(本月vs上月Top1)·记录时段偏好(最常何时记)·本月情绪转换统计(最常见A→B转变链)/今日未记录轻提醒/未打卡可一键回顶部记录/健康指数❓说明(含较上周对比·修复说明文案不显示bug)·本月记录天数徽章·情绪断崖预警/三件小事(💡灵感一键填入·四场景/勾选记完成时刻·连续全完成天数🔥)·全完成庆祝/记录后暖心回应+负性情绪「情绪急救包」(3件即时小事·一键去呼吸/充电站)·分布一句话解读/最像此刻的一天回看/时光信）
...
│   ├── helper/    求助（热线(最近拨过的自动排前·长按卡片复制号码·🕘最近拨打记录(带相对时间·鼓励语轮换)·🫁开口前先做60秒呼吸直达)/安全包·我提前信任的人名单(支持存电话·危急一键拨打·完整度提示(N人M电话)·30天未更新保鲜提醒·长按清号码·📤导出/📥剪贴板恢复)/1h·24h·3d·7d四段温暖回访/走完四段一次性「毕业卡」致谢/回访关怀卡/求助后静音关怀(可提前手动结束)/智能分流提示/「现在最需要什么」三方向快速分流/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/开口求助话术生成器(9种处境·默认显示4个可展开全部→生成第一句话·可复制/✏️改一改再复制/换说法·最近复制过可再复制·含被欺负/家里吵架)+考试季家长贴士一键复制转发/🫂「帮助朋友」危机卡(怎么陪·怎么问安全·一键复制)/一键复制求助文字发信任的人/今日关怀/英语角）
│   ├── breathe/   呼吸放松练习（4 轮引导/4 档节奏 4-4-6·4-4-4-4·6-2-6-2+发呆2分钟(累计次数陪伴计数)+自定义节奏(吸/屏/呼 2-10 秒可调·一键填入4-7-8助眠/4-4-6/6-2-6)+按时段推荐节奏(晨盒式/午放松/夜长呼)/练习中断24h内温柔回访提示·结束可选 1-5 强度记录平静(发呆模式自动隐藏强度·文案专属)/呼吸徽章/累计+本周进度统计·每日 10 分钟目标进度条+达标连击+近4周放松热力图+完成时段分布(早/中/晚/深夜)+本周vs上周次数对比+最近7次放松回看·完成练习「呼吸回响」(发呆模式结束自动给专属回响·→直达心理小课为什么呼吸有用)，全程本地）
│   ├── scan/      5 分钟身体扫描（完成后可接续呼吸·按时段扫描建议·10 段引导计时·完成自动记「平静」+可选 1-5 强度+身体感受一词(8词含累了·最常完成时段统计·留最近5次回看·🫧最常反馈Top1提示)/累计完成次数/再来一次/每日一句轮换开场）
│   ├── station/   情绪充电站（按此刻心情筛选(焦虑/难过/生气/睡前/学习/平静)·12 张可展开心理急救卡(展开自动记阅读次数·📖最常翻Top1提示·阅读总数鼓励(≥5次))(含拖延启动式·睡前两行清单·发呆 2 分钟·气得发抖时 60 秒降火·5-4-3-2-1接地术·考前一夜3件事·情绪词库准确命名)/一键跳呼吸·聊天·记下此刻心情·「🎴随机抽一张」(第N次抽中缘分提示)·卡片长按★收藏置顶(最多3张·收藏概览提示)·展开可📄复制单张·入口「心理小课」/可分享·今日一句话(换日轮换·每日可手动换一次)）
...
│   └── profile/   我的（数据足迹含年度统计(今年N条·覆盖N天)+心理小课进度条直达/成就徽章×16(点亮率百分比·🔥连学7天·已点亮自动置前并记录点亮日期·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课7节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·覆盖全部40+本地键·云端心情与徽章保留)/复制文字版「我的概览」(含结业与温故题数)+「数据都放在哪」透明说明/数据足迹(近7天格子带星期标签·本月记录天数进度条)+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏分组展示(今天/更早·可一键复制全部)/按时段日签问候+累计陪伴天数(7/30/100/365天里程碑文案·可复制留念)·最近自评直达/好久没回温柔卡）
