...
├── utils/api.js                      # 云函数封装
├── pages/
│   ├── welcome/   隐私 & AI 说明 + 首次 3 步新手引导卡(可点直达对应页·可跳过) + 🆕最近更新日志(可折叠·已更新至v2.0)
...
│   └── profile/   我的（数据足迹含年度统计(今年N条·覆盖N天)+心理小课进度条直达/成就徽章×16(点亮率百分比·🔥连学7天·已点亮自动置前并记录点亮日期·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课8节)/今日心情卡(Canvas生成含连续打卡天数·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·覆盖全部50+本地键·云端心情与徽章保留)/复制文字版「我的概览」(含结业与温故题数·末尾附热线提醒)+「数据都放在哪」透明说明/数据足迹(近7天格子·本月天数进度·呼吸+扫描总计·晚间小结篇数)+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏分组展示(今天/更早·可一键复制全部)/按时段日签问候+累计陪伴天数(7/30/100/365天里程碑文案·可复制留念)·最近自评直达/好久没回温柔卡）
