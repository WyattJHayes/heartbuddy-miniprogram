...
│   ├── breathe/   呼吸放松练习（4 轮引导/4 档节奏 4-4-6·4-4-4-4·6-2-6-2+发呆2分钟+自定义节奏(吸/屏/呼 2-10 秒可调·本地保存)+按时段推荐节奏(晨盒式/午放松/夜长呼)·结束可选 1-5 强度记录平静/呼吸徽章/累计+本周进度统计·每日 10 分钟目标进度条+达标连击+近4周放松热力图+最近7次放松回看·完成练习「呼吸回响」(发呆模式结束自动给专属回响·→直达心理小课为什么呼吸有用)，全程本地）
│   ├── scan/      5 分钟身体扫描（按时段扫描建议·10 段引导计时·完成自动记「平静」+可选 1-5 强度+身体感受一词(松了/暖了…留最近5次回看)/累计完成次数/再来一次/每日一句轮换开场）
│   ├── station/   情绪充电站（按此刻心情筛选(焦虑/难过/生气/睡前/学习/平静)·12 张可展开心理急救卡(含拖延启动式·睡前两行清单·发呆 2 分钟·气得发抖时 60 秒降火·5-4-3-2-1接地术·考前一夜3件事·情绪词库准确命名)/一键跳呼吸·聊天·记下此刻心情·「🎴随机抽一张」·卡片长按★收藏置顶(最多3张)·展开可📄复制单张·入口「心理小课」/可分享·今日一句话(换日轮换·每日可手动换一次)）
│   ├── edu/       心理小课（7 节心理健康小课程·本地：认识情绪/考前焦虑/睡不着/低落最小行动/朋友闹掰/社媒比较心/向大人求助不丢人·打开自动定位第一节未学(续学)·每课📄一键复制分享·随堂一问答对自动点亮·完成度进度条·🔥连续学习天数·✏️学习心得一句话(本地)·学完7节出结业卡(结业词可复制)·每日温故一题(答错进错题本·温故优先抽错题)·一键重置·热线直达）
