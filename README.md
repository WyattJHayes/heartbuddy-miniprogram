...
├── pages/
│   ├── welcome/   隐私 & AI 说明
│   ├── chat/        核心对话页（快捷情绪双闭环/此刻强度滑条/倾诉后小结算/AI头像情绪态/打字接续草稿/深夜工具箱·写给自己/回复引用/长按AI回复「换个说法」重新生成/连续低落情绪关怀条(可顺手写给此刻的自己)/高危信号首次强调认真对待/自我安抚快捷短句/撤回我的消息/输入框按时段共鸣占位/英文角/天气/今日心语/晨间轻提醒/计划日提示/重新开始对话/长按珍藏·含撤回/今日第N次倾诉徽章）
│   ├── mood/      情绪可视化（快速记录/8种心情/健康指数/14天洞察·连低连高解读/色带点天小结+写一句补记+长按管理/日历+长按管理·空白日可补记/今日未记录轻提醒/未打卡可一键回顶部记录/健康指数❓说明/三件小事·全完成庆祝/记录后暖心回应·分布一句话解读/时光信）
│   ├── assessment/ 考前焦虑自评 + 应对卡 + 分享给信任的人(脱敏复制) + 逐题洞察 + 历史波动提醒 + 3 天陪伴计划 + 距上次测评轻提醒 + 结果页引导·去充电站 + 高分就医指引
│   ├── ops/       运营看板（危机闭环/本周与月度处理率/月度对比行/今日新增危机快报/用户反馈流·点开全文/CSV/7日均值快照/来源分布/高危趋势+24h回访闭环/高危处理率近7日趋势/高危用户带最近高危时间/高危24小时时段分布/高危清单一键复制脱敏列表/反馈一键标记已回应+全部批量/修复）
