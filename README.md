...
│   ├── scan/      5 分钟身体扫描（完成后可接续呼吸·按时段扫描建议·10 段引导计时·完成自动记「平静」+可选 1-5 强度+身体感受一词(松了/暖了/轻了/稳了等7词·留最近5次回看)/累计完成次数/再来一次/每日一句轮换开场）
...
│   ├── edu/       心理小课（课程带第N节序号/🔍课程内容搜索(无结果友好提示)/7 节心理健康小课程·本地：认识情绪/考前焦虑/睡不着/低落最小行动/朋友闹掰/社媒比较心/向大人求助不丢人·打开自动定位第一节未学(续学)·每课📄一键复制分享(带学习进度)·随堂一问答对自动点亮并自动展开下一节·完成度进度条·🔥连续学习天数+近14天学习日历·✏️学习心得一句话(本地·可复制)·学完7节出结业卡(记结业日期·结业词可复制·页面可直接分享)·每日温故一题(答错进错题本·优先抽错题·结业卡展示累计答对题数)·一键重置·热线直达）
│   ├── privacy/   隐私政策（全文·含「如何删除我的数据」自助指引：单条删除/清演示/本地缓存/云端记录说明）
│   └── profile/   我的（数据足迹含年度统计(今年N条·覆盖N天)+心理小课进度条直达/成就徽章×16(🔥连学7天·已点亮自动置前并记录点亮日期·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课7节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·覆盖全部30+本地键·云端心情与徽章保留)/复制文字版「我的概览」(含结业与温故题数)+「数据都放在哪」透明说明/数据足迹(近7天格子带星期标签)+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候+累计陪伴天数(7/30/100/365天里程碑文案·可复制留念)·最近自评直达/好久没回温柔卡）
