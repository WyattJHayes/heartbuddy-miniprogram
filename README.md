...
│   ├── edu/       心理小课（7 节心理健康小课程·本地：认识情绪/考前焦虑/睡不着/低落最小行动/朋友闹掰/社媒比较心/向大人求助不丢人·打开自动定位第一节未学(续学)·每课「随堂一问」答对自动点亮·完成度进度条·🔥连续学习天数·✏️学习心得一句话(本地)·学完7节出结业卡(结业词可复制)·一键重置·热线直达）
│   ├── privacy/   隐私政策（全文·含「如何删除我的数据」自助指引：单条删除/清演示/本地缓存/云端记录说明）
│   └── profile/   我的（数据足迹含心理小课进度条直达/成就徽章×14(🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课7节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)+「数据都放在哪」透明说明/数据足迹+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候+累计陪伴天数·最近自评直达/好久没回温柔卡）
