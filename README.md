...
│   ├── chat/        核心对话页（⭐我的常用语(本地存5条·点按填入长按删)/夜间柔和模式(22:00-6:00 自动深色护眼)/快捷情绪双闭环/此刻强度滑条(7种情绪标签含🌟期待)/倾诉后小结算/AI头像情绪态/打字接续草稿/深夜工具箱·写给自己+5分钟急救(固定预案)+睡前托一句话明早回读/📋一键复制整段对话留档(带时间戳与主题行)/回复引用/长按AI回复「换个说法」重新生成/一键总结这轮对话(总结后引导顺手去心情页记一笔)/长对话折叠·一键收起/连续低落情绪关怀条(可顺手写给此刻的自己)/周一晨间上周陪伴小结(条数·天数·主要情绪·每周一次)/高危信号首次强调认真对待/自我安抚快捷短句/夸夸我本地词库+和我做3次深呼吸即时引导/撤回我的消息/输入框按时段共鸣占位/英文角/天气/今日心语/晨间轻提醒/计划日提示/重新开始对话/长按珍藏·含撤回/今日第  N 次倾诉徽章/陪伴身份(朋友/树洞/清醒教练·本地记住)/陪伴里程碑(111·333·999句)/底栏已聊几 句·写字数计数）
...
│   ├── helper/    求助（热线/安全包·我提前信任的人名单(支持存电话·危急一键拨打)/1h·24h·3d·7d四段温暖回访/走完四段一次性「毕业卡」致谢/回访关怀卡/求助后静音关怀/智能分流提示/「现在最需要什么」三方向快速分流/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/开口求助话术生成器(7种处境→生成第一句话·可复制/✏️改一改再复制/换说法·含被欺负/家里吵架)+考试季家长贴士一键复制转发/一键复制求助文字发信任的人/今日关怀/英语角）
...
│   ├── station/   情绪充电站（12 张可展开心理急救卡(含拖延启动式·睡前两行清单·发呆 2 分钟·气得发抖时 60 秒降火·5-4-3-2-1接地术·考前一夜3件事·情绪词库准确命名)/一键跳呼吸·聊天·记下此刻心情·「🎴随机抽一张」·卡片长按★收藏置顶(最多3张)·入口「心理小课」/可分享·今日一句话(换日轮换·每日可手动换一次)）
