...
├── pages/
│   ├── welcome/   隐私 & AI 说明
│   ├── chat/        核心对话页（快捷情绪双闭环/此刻强度滑条/倾诉后小结算/AI头像情绪态/打字接续草稿/深夜工具箱·写给自己/回复引用/连续低落情绪关怀条/自我安抚快捷短句/撤回我的消息/英文角/天气/今日心语/晨间轻提醒/计划日提示/重新开始对话/长按珍藏·含撤回）
│   ├── mood/      情绪可视化（快速记录/8种心情/健康指数/14天洞察·连低连高解读/色带点天小结+写一句补记+长按管理/日历+长按管理/今日未记录轻提醒/三件小事/时光信）
│   ├── assessment/ 考前焦虑自评 + 应对卡 + 分享给信任的人(脱敏复制) + 逐题洞察 + 历史波动提醒 + 3 天陪伴计划 + 距上次测评轻提醒 + 结果页引导·去充电站
│   ├── ops/       运营看板（危机闭环/本周与月度处理率/月度对比行/今日新增危机快报/用户反馈流·点开全文/CSV/7日均值快照/来源分布/高危趋势+24h回访闭环/高危处理率近7日趋势）
│   ├── report/    周报 + 月度小结（Top3/上月对比/平均强度/情绪词云）+ 连续低日提醒 + 正向鼓励条 + 本周相对最好的一天 + 本周对自己说备注 + canvas 分享卡
│   ├── helper/    求助（热线/24h回访/回访关怀卡/求助后静音关怀/智能分流提示/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/今日关怀/英语角）
│   ├── breathe/   呼吸放松练习（4 轮引导/3 档节奏 4-4-6·4-4-4-4·6-2-6-2·结束记录平静/呼吸徽章/累计练习统计）
│   ├── scan/      5 分钟身体扫描（10 段引导计时·完成自动记「平静」/累计完成次数）
│   ├── station/   情绪充电站（5 张可展开心理急救卡/一键跳呼吸·聊天/可分享）
│   ├── scan/      5 分钟身体扫描（10 段引导计时·完成自动记「平静」/累计完成次数）
│   ├── privacy/   隐私政策（全文）
│   └── profile/   我的（成就徽章×13/导出数据/数据足迹+月均强度·常用入口/珍藏长按删单条）
