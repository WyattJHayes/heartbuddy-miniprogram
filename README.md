...
│   ├── chat/        核心对话页（⭐我的常用语(本地存5条·点按填入长按删)/夜间柔和模式(22:00-6:00 自动深色护眼)/快捷情绪双闭环/此刻强度滑条(7种情绪标签含🌟期待)/倾诉后小结算/AI头像情绪态/打字接续草稿/深夜工具箱·写给自己+5分钟急救(固定预案)+睡前托一句话明早回读/📋一键复制整段对话留档(带时间戳与主题行)/回复引用/长按AI回复「换个说法」重新生成/一键总结这轮对话(总结后引导顺手去心情页记一笔)/长对话折叠·一键收起·消息跨天日期分隔线(今天/昨天/M月D日)/连续低落情绪关怀条(可顺手写给此刻的自己)/周一晨间上周陪伴小结(条数·天数·主要情绪·每周一次)/高危信号首次强调认真对待/自我安抚快捷短句/夸夸我本地词库+和我做3次深呼吸即时引导/撤回我的消息/输入框按时段共鸣占位/英文角/天气/今日心语/晨间轻提醒/计划日提示/重新开始对话/长按珍藏·含撤回/今日第  N 次倾诉徽章/陪伴身份(朋友/树洞/清醒教练·本地记住)/陪伴里程碑(111·333·999句)/底栏已聊几 句·写字数计数）
...
│   ├── helper/    求助（热线/安全包·我提前信任的人名单(支持存电话·危急一键拨打)/1h·24h·3d·7d四段温暖回访/走完四段一次性「毕业卡」致谢/回访关怀卡/求助后静音关怀/智能分流提示/「现在最需要什么」三方向快速分流/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/开口求助话术生成器(7种处境→生成第一句话·可复制/✏️改一改再复制/换说法·最近复制过可再复制·含被欺负/家里吵架)+考试季家长贴士一键复制转发/🫂「帮助朋友」危机卡(怎么陪·怎么问安全·一键复制)/一键复制求助文字发信任的人/今日关怀/英语角）
...
│   └── profile/   我的（数据足迹含心理小课进度条直达/成就徽章×16(新增🔥连学7天·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课7节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·云端心情与徽章保留)/复制文字版「我的概览」+「数据都放在哪」透明说明/数据足迹+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候+累计陪伴天数·最近自评直达/好久没回温柔卡）
