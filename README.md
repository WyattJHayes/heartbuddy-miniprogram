...
│   ├── mood/      情绪可视化（本周小结一键复制/快速记录(强度1-5可选·默认3)/记录里程碑庆祝(第10/50/100/200条一次性)/8种心情/健康指数/14天洞察(色带14/30天⇄切换)·连低连高解读·曲线今日锚点/色带点天小结+写一句补记+长按管理/日历+翻月回本月·本月触发来源排行·本月情绪转换统计(最常见A→B转变链)/今日未记录轻提醒/未打卡可一键回顶部记录/健康指数❓说明(含较上周对比·修复说明文案不显示bug)·本月记录天数徽章·情绪断崖预警/三件小事(💡灵感一键填入·四场景:学习恢复/身体照顾/与人连接/独处时光)·全完成庆祝/记录后暖心回应+负性情绪「情绪急救包」(3件即时小事·一键去呼吸/充电站)·分布一句话解读/最像此刻的一天回看/时光信）
...
│   ├── edu/       心理小课（🔍课程内容搜索/7 节心理健康小课程·本地：认识情绪/考前焦虑/睡不着/低落最小行动/朋友闹掰/社媒比较心/向大人求助不丢人·打开自动定位第一节未学(续学)·每课📄一键复制分享·随堂一问答对自动点亮·完成度进度条·🔥连续学习天数·✏️学习心得一句话(本地)·学完7节出结业卡(结业词可复制)·每日温故一题(答错进错题本·温故优先抽错题)·一键重置·热线直达）
│   ├── privacy/   隐私政策（全文·含「如何删除我的数据」自助指引：单条删除/清演示/本地缓存/云端记录说明）
│   └── profile/   我的（数据足迹含心理小课进度条直达/成就徽章×16(🔥连学7天·已点亮自动置前·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课7节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·云端心情与徽章保留)/复制文字版「我的概览」+「数据都放在哪」透明说明/数据足迹+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候+累计陪伴天数·最近自评直达/好久没回温柔卡）
