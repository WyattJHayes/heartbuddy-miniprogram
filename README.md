...
│   ├── report/    周报 + 月度小结（本月vs上月条数对比·Top3/上月对比/平均强度/情绪词云·📋一键复制月度小结可发老师家长）+ 本周情绪构成彩带(堆叠条+图例含次数·一句话解读)+ 连续低日提醒 + 正向鼓励条 + 一句话状态总结 + 本周晚间小结篇数+本周相对最好的一天 + 本周练习足迹 + 文字版含练习足迹与晚间小结篇数 + 本周平均强度 + 本周对自己说备注 + 近4周记录节奏 + canvas 分享卡
...
│   ├── edu/       心理小课（课程带第N节序号/🔍课程内容搜索(无结果友好提示)/8 节心理健康小课程·本地：认识情绪/考前焦虑/睡不着/低落最小行动/朋友闹掰/社媒比较心/向大人求助不丢人/和父母好好说话·打开自动定位第一节未学(续学)·每课📄一键复制分享(带学习进度)·随堂一问答对自动点亮并自动展开下一节·选项随机打乱防背位置·完成度进度条·🔥连续学习天数+近14天学习日历+每课重学遍数·✏️学习心得一句话(本地·可复制)·学完8节出结业卡(记结业日期与学习总天数·结业词可复制·页面可直接分享)·每日温故一题(答错进错题本·优先抽错题·结业卡展示答对题数与正确率+温故连续天数🔥)·一键重置·热线直达）
│   ├── privacy/   隐私政策（全文·含「如何删除我的数据」自助指引：单条删除/清演示/本地缓存/云端记录说明）
│   └── profile/   我的（数据足迹含年度统计(今年N条·覆盖N天)+心理小课进度条直达/成就徽章×16(点亮率百分比·🔥连学7天·已点亮自动置前并记录点亮日期·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课8节)/今日心情卡(Canvas生成含连续打卡天数·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·覆盖全部50+本地键·云端心情与徽章保留)/复制文字版「我的概览」(含结业与温故题数·末尾附热线提醒)+「数据都放在哪」透明说明/数据足迹(近7天格子·本月天数进度·呼吸+扫描总计)+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏分组展示(今天/更早·可一键复制全部)/按时段日签问候+累计陪伴天数(7/30/100/365天里程碑文案·可复制留念)·最近自评直达/好久没回温柔卡）
