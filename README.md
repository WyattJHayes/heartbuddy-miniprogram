...
│   ├── helper/    求助（热线/安全包·我提前信任的人名单/1h·24h·3d·7d四段温暖回访/回访关怀卡/求助后静音关怀/智能分流提示/「现在最需要什么」三方向快速分流/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/一键复制求助文字发信任的人/今日关怀/英语角）
│   ├── breathe/   呼吸放松练习（4 轮引导/3 档节奏 4-4-6·4-4-4-4·6-2-6-2+发呆2分钟·结束可选 1-5 强度记录平静/呼吸徽章/累计+本周进度统计·每日 10 分钟目标进度条+达标连击+最近7次放松回看·完成练习「呼吸回响」，全程本地）
│   ├── scan/      5 分钟身体扫描（10 段引导计时·完成自动记「平静」+可选 1-5 强度/累计完成次数/再来一次/每日一句轮换开场）
│   ├── station/   情绪充电站（10 张可展开心理急救卡(含拖延启动式·睡前两行清单·发呆 2 分钟·气得发抖时 60 秒降火·5-4-3-2-1接地术)/一键跳呼吸·聊天·记下此刻心情·入口「心理小课」/可分享·今日一句话(换日轮换)）
│   ├── edu/       心理小课（4 节心理健康小课程·本地：认识情绪/考前焦虑/睡不暖/低落最小行动·「我学完啦」打卡记完成度进度条·一键重置·热线直达）
│   ├── privacy/   隐私政策（全文）
│   └── profile/   我的（成就徽章×13/今日心情卡(Canvas生成·可保存分享)/导出数据+「数据都放在哪」透明说明/数据足迹+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候+累计陪伴天数·最近自评直达/好久没回温柔卡）
