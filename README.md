...
│   ├── station/   情绪充电站（按此刻心情筛选(焦虑/难过/生气/睡前/学习/平静)·12 张可展开心理急救卡(展开自动记阅读次数)(含拖延启动式·睡前两行清单·发呆 2 分钟·气得发抖时 60 秒降火·5-4-3-2-1接地术·考前一夜3件事·情绪词库准确命名)/一键跳呼吸·聊天·记下此刻心情·「🎴随机抽一张」·卡片长按★收藏置顶(最多3张·收藏概览提示)·展开可📄复制单张·入口「心理小课」/可分享·今日一句话(换日轮换·每日可手动换一次)）
│   ├── edu/       心理小课（课程带第N节序号/🔍课程内容搜索(无结果友好提示)/7 节心理健康小课程·本地：认识情绪/考前焦虑/睡不着/低落最小行动/朋友闹掰/社媒比较心/向大人求助不丢人·打开自动定位第一节未学(续学)·每课📄一键复制分享·随堂一问答对自动点亮·完成度进度条·🔥连续学习天数·✏️学习心得一句话(本地·可复制)·学完7节出结业卡(结业词可复制)·每日温故一题(答错进错题本·温故优先抽错题)·一键重置·热线直达）
│   ├── privacy/   隐私政策（全文·含「如何删除我的数据」自助指引：单条删除/清演示/本地缓存/云端记录说明）
│   └── profile/   我的（数据足迹含年度统计(今年N条·覆盖N天)+心理小课进度条直达/成就徽章×16(🔥连学7天·已点亮自动置前·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课7节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·云端心情与徽章保留)/复制文字版「我的概览」+「数据都放在哪」透明说明/数据足迹+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候+累计陪伴天数(7/30/100/365天里程碑文案)·最近自评直达/好久没回温柔卡）
