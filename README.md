...
├── pages/
│   ├── welcome/   隐私 & AI 说明
│   ├── chat/        核心对话页（快捷情绪双闭环/此刻强度滑条/倾诉后小结算/AI头像情绪态/打字接续草稿/深夜工具箱·写给自己/回复引用/长按AI回复「换个说法」重新生成/一键总结这轮对话/连续低落情绪关怀条(可顺手写给此刻的自己)/高危信号首次强调认真对待/自我安抚快捷短句/撤回我的消息/输入框按时段共鸣占位/英文角/天气/今日心语/晨间轻提醒/计划日提示/重新开始对话/长按珍藏·含撤回/今日第N次倾诉徽章）
...
│   ├── privacy/   隐私政策（全文）
│   └── profile/   我的（成就徽章×13/导出数据/数据足迹+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候）
