...
| 🌱 心情 | 情绪记录与可视化（8 种情绪+触发来源快选·打卡曲线+健康指数 / 心情日历 / 时光信·已拆开收进「信匣」可回看 / 想法小剧场(念头存7天·到期回看·入库想法盒) / 连续空白温柔提醒(≥3天·不评判)/21点晚安条(一次性)/今日小结(21点后收尾·次日回看可一键复制)·📖小结日记本(近7天/本月/全部三档筛选·单条可复制)） |
...
│   ├── mood/      情绪可视化（本周小结一键复制/快速记录(强度1-5可选·toast带本周第N条实感)/记录里程碑庆祝(第10/50/100/200条一次性)/8种心情/健康指数/14天洞察(色带14/30天⇄切换·8情绪图例·今天描边高亮·空白天虚线区分·🏆最长连记天数)·连低连高解读·曲线今日锚点/色带点天小结+写一句补记+长按管理/日历+翻月回本月·当月最长连续记录段+当月最强情绪日提示/📋一键复制月报(条数/天数/Top3/晚间小结篇数)·本月触发来源排行·环比洞察(本月vs上月Top1)·记录时段偏好(最常何时记)·本月情绪转换统计(最常见A→B转变链)/今日未记录轻提醒/未打卡可一键回顶部记录/健康指数❓说明(含较上周对比·修复说明文案不显示bug)·本月记录天数徽章·情绪断崖预警/三件小事(💡灵感一键填入·四场景/勾选记完成时刻·连续全完成天数🔥)·全完成庆祝/记录后暖心回应+负性情绪「情绪急救包」(3件即时小事·一键去呼吸/充电站)·分布一句话解读/最像此刻的一天回看/时光信）
...
│   ├── report/    周报 + 月度小结（Top3/上月对比/平均强度/情绪词云·📋一键复制月度小结可发老师家长）+ 本周情绪构成彩带(堆叠条+图例含次数·一句话解读)+ 连续低日提醒 + 正向鼓励条 + 一句话状态总结 + 本周相对最好的一天 + 本周练习足迹 + 文字版含练习足迹 + 本周平均强度 + 本周对自己说备注 + 近4周记录节奏 + canvas 分享卡
