...
| 🌱 心情 | 情绪记录与可视化（8 种情绪+触发来源快选·打卡曲线+健康指数 / 心情日历 / 时光信·已拆开收进「信匣」可回看 / 想法小剧场(念头存7天·到期回看·入库想法盒) / 连续空白温柔提醒(≥3天·不评判)/21点晚安条(一次性)/今日小结(21点后收尾·次日回看可一键复制)） |
...
│   ├── station/   情绪充电站（11 张可展开心理急救卡(含拖延启动式·睡前两行清单·发呆 2 分钟·气得发抖时 60 秒降火·5-4-3-2-1接地术·考前一夜3件事)/一键跳呼吸·聊天·记下此刻心情·「🎴随机抽一张」·卡片长按★收藏置顶(最多3张)·入口「心理小课」/可分享·今日一句话(换日轮换·每日可手动换一次)）
...
│   └── profile/   我的（数据足迹含心理小课进度条直达/成就徽章×15(新增📣开口一次·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课7节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)+「数据都放在哪」透明说明/数据足迹+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候+累计陪伴天数·最近自评直达/好久没回温柔卡）
