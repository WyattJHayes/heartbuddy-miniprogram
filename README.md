...
| 🌱 心情 | 情绪记录与可视化（8 种情绪+触发来源快选·打卡曲线+健康指数 / 心情日历 / 时光信·已拆开收进「信匣」可回看 / 想法小剧场(念头存7天·第6天「明天就能看」提醒·到期回看·入库想法盒) / 连续空白温柔提醒(≥3天·不评判)/21点晚安条(一次性)/今日小结(21点后收尾·次日回看可一键复制)·📖小结日记本(近7天/本月/全部三档筛选·单条可复制)） |
...
├── utils/api.js                      # 云函数封装
├── pages/
│   ├── welcome/   隐私 & AI 说明 + 首次 3 步新手引导卡(可点直达对应页·可跳过) + 🆕最近更新日志(可折叠·已更新至v1.8)
...
│   ├── station/   情绪充电站（未读卡片NEW角标·按此刻心情筛选(焦虑/难过/生气/睡前/学习/平静)·12 张可展开心理急救卡(展开自动记阅读次数·📖最常翻Top1提示·阅读总数鼓励(≥5次))(含拖延启动式·睡前两行清单·发呆 2 分钟·气得发抖时 60 秒降火·5-4-3-2-1接地术·考前一夜3件事·考试当天早晨5分钟·情绪词库准确命名)/一键跳呼吸·聊天·记下此刻心情·「🎴随机抽一张」(第N次抽中缘分提示)·卡片长按★收藏置顶(最多3张·收藏概览提示)·展开可📄复制单张·入口「心理小课」/可分享·今日一句话(换日轮换·每日可手动换一次)）
