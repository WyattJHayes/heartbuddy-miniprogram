...
| 🌱 心情 | 情绪记录与可视化（8 种情绪+触发来源快选·打卡曲线+健康指数 / 心情日历 / 时光信·已拆开收进「信匣」可回看 / 想法小剧场(念头存7天·到期回看·入库想法盒)） |
...
│   ├── breathe/   呼吸放松练习（4 轮引导/3 档节奏 4-4-6·4-4-4-4·6-2-6-2+发呆2分钟·结束可选 1-5 强度记录平静/呼吸徽章/累计+本周进度统计·每日 10 分钟目标进度条+达标连击+最近7次放松回看，全程本地）
│   ├── scan/      5 分钟身体扫描（10 段引导计时·完成自动记「平静」+可选 1-5 强度/累计完成次数/再来一次/每日一句轮换开场）
│   ├── station/   情绪充电站（10 张可展开心理急救卡(含拖延启动式·睡前两行清单·发呆 2 分钟·气得发抖时 60 秒降火·5-4-3-2-1接地术)/一键跳呼吸·聊天·记下此刻心情/可分享·今日一句话(换日轮换)）
│   ├── privacy/   隐私政策（全文）
│   └── profile/   我的（成就徽章×13/导出数据+「数据都放在哪」透明说明/数据足迹+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候+累计陪伴天数·最近自评直达/好久没回温柔卡）
