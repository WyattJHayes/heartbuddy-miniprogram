...
│   ├── helper/    求助（热线(长按卡片复制号码·🕘最近拨打记录鼓励性回显·🫁开口前先做60秒呼吸直达)/安全包·我提前信任的人名单(支持存电话·危急一键拨打·长按清号码·📤导出/📥剪贴板恢复)/1h·24h·3d·7d四段温暖回访/走完四段一次性「毕业卡」致谢/回访关怀卡/求助后静音关怀(可提前手动结束)/智能分流提示/「现在最需要什么」三方向快速分流/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/开口求助话术生成器(7种处境→生成第一句话·可复制/✏️改一改再复制/换说法·最近复制过可再复制·含被欺负/家里吵架)+考试季家长贴士一键复制转发/🫂「帮助朋友」危机卡(怎么陪·怎么问安全·一键复制)/一键复制求助文字发信任的人/今日关怀/英语角）
...
│   ├── edu/       心理小课（课程带第N节序号/🔍课程内容搜索(无结果友好提示)/7 节心理健康小课程·本地：认识情绪/考前焦虑/睡不着/低落最小行动/朋友闹掰/社媒比较心/向大人求助不丢人·打开自动定位第一节未学(续学)·每课📄一键复制分享·随堂一问答对自动点亮·完成度进度条·🔥连续学习天数·✏️学习心得一句话(本地·可复制)·学完7节出结业卡(结业词可复制)·每日温故一题(答错进错题本·优先抽错题·结业卡展示累计答对题数)·一键重置·热线直达）
│   ├── privacy/   隐私政策（全文·含「如何删除我的数据」自助指引：单条删除/清演示/本地缓存/云端记录说明）
│   └── profile/   我的（数据足迹含年度统计(今年N条·覆盖N天)+心理小课进度条直达/成就徽章×16(🔥连学7天·已点亮自动置前并记录点亮日期·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课7节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·云端心情与徽章保留)/复制文字版「我的概览」+「数据都放在哪」透明说明/数据足迹+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏可一键复制全部/按时段日签问候+累计陪伴天数(7/30/100/365天里程碑文案)·最近自评直达/好久没回温柔卡）
