...
│   ├── helper/    求助（热线(最近拨过的自动排前·长按卡片复制号码·🕘最近拨打记录(带相对时间·鼓励语轮换)·🫁开口前先做60秒呼吸直达)/安全包·我提前信任的人名单(支持存电话·危急一键拨打·✏️为什么信任备注·完整度提示(N人M电话)·30天未更新保鲜提醒·长按清号码·📤导出(含信任备注)/📥剪贴板恢复)/1h·24h·3d·7d四段温暖回访/走完四段一次性「毕业卡」致谢/回访关怀卡/求助后静音关怀(可提前手动结束)/智能分流提示/「现在最需要什么」三方向快速分流/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/开口求助话术生成器(9种处境·默认显示4个可展开全部→生成第一句话·可复制/✏️改一改再复制/换说法·最近复制过可再复制·含被欺负/家里吵架)+考试季家长贴士一键复制转发/🫂「帮助朋友」危机卡(怎么陪·怎么问安全·一键复制)/一键复制求助文字发信任的人/今日关怀/英语角）
...
│   ├── edu/       心理小课（课程带第N节序号/🔍课程内容搜索(无结果友好提示)/8 节心理健康小课程·本地：认识情绪/考前焦虑/睡不着/低落最小行动/朋友闹掰/社媒比较心/向大人求助不丢人/和父母好好说话·打开自动定位第一节未学(续学)·每课📄一键复制分享(带学习进度)·随堂一问答对自动点亮并自动展开下一节·选项随机打乱防背位置·完成度进度条·🔥连续学习天数+近14天学习日历+每课重学遍数·✏️学习心得一句话(本地·可复制)·学完7节出结业卡(记结业日期·结业词可复制·页面可直接分享)·每日温故一题(答错进错题本·优先抽错题·结业卡展示答对题数与正确率+温故连续天数🔥)·一键重置·热线直达）
