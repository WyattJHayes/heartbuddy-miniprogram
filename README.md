...
| 模块 | 说明 |
|---|---|
| 💬 陪伴 | AI 多轮情绪对话（流式打字机），可标记当前心情；**首次登录个性化欢迎语** |
| 🌱 心情 | 情绪记录与可视化（占比条 + 最近） |
...
├── pages/
│   ├── welcome/   隐私 & AI 说明
│   ├── chat/      核心对话页（快捷情绪双闭环/英文角/天气角/今日心语/长按复制珍藏）
│   ├── mood/      情绪可视化（快速记录/打卡曲线+健康指数/心情日历签到/时光信/提醒/日记删除）
│   ├── assessment/ 考前焦虑自评 + 应对卡导出 + 历史趋势逐题洞察 + 3 天陪伴计划
│   ├── ops/       运营看板（危机闭环/处理率/CSV/7日均值快照/来源分布/高危趋势+24h回访闭环）
│   ├── report/    周报 + canvas 分享卡（均值对比+7天折线）+ 复制文字版/往期回看
│   ├── helper/    求助中心（热线/24h回访/呼吸放松/「我撑过来了」5步自救/今日关怀/英语角）
