...
├── pages/
│   ├── welcome/   隐私 & AI 说明
│   ├── chat/      核心对话页（快捷情绪双闭环·英文角/新手引导/实时天气角）
│   ├── mood/      情绪可视化（快速记录/打卡 streak/曲线+小结图/今日任务/本周小结/时光信/每日提醒/演示模式/情绪日记：详情+删除）
...
│   ├── helper/    求助中心（热线/已安全回执·24h 回访/呼吸放松入口/今日关怀/英语角）
│   ├── breathe/   呼吸放松练习（4 轮引导·结束记录平静/呼吸徽章计数）
│   ├── privacy/   隐私政策（全文）
│   └── profile/   我的（成就徽章×9/一键导出本人数据/数据足迹）
