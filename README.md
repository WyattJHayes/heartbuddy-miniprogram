...
├── pages/
│   ├── welcome/   隐私 & AI 说明
│   ├── chat/        核心对话页（快捷情绪双闭环/此刻强度滑条/倾诉后小结算/AI头像情绪态/打字接续草稿/深夜工具箱·写给自己+5分钟急救(固定预案)+睡前托一句话明早回读/回复引用/长按AI回复「换个说法」重新生成/一键总结这轮对话/连续低落情绪关怀条(可顺手写给此刻的自己)/高危信号首次强调认真对待/自我安抚快捷短句/夸夸我本地词库+和我做3次深呼吸即时引导/撤回我的消息/输入框按时段共鸣占位/英文角/天气/今日心语/晨间轻提醒/计划日提示/重新开始对话/长按珍藏·含撤回/今日第N次倾诉徽章/底栏已聊几句·写字数计数）
│   ├── mood/      情绪可视化（快速记录/8种心情/健康指数/14天洞察·连低连高解读·曲线今日锚点/色带点天小结+写一句补记+长按管理/日历+长按管理·空白日可补记/今日未记录轻提醒/未打卡可一键回顶部记录/健康指数❓说明·本月记录天数徽章/三件小事·全完成庆祝/记录后暖心回应·分布一句话解读/最像此刻的一天回看/时光信）
│   ├── assessment/ 考前焦虑自评 + 应对卡 + 分享给信任的人(脱敏复制) + 逐题洞察 + 历史波动提醒 + 3 天陪伴计划 + 距上次测评轻提醒 + 结果页引导·去充电站 + 高分就医指引 + 3天后复查提醒
│   ├── ops/       运营看板（危机闭环/本周与月度处理率/月度对比行/累计用户·今日新增用户·7日活跃/今日新增危机快报/用户反馈流·点开全文/CSV/7日均值快照/来源分布/高危趋势+24h回访闭环/高危处理率近7日趋势/高危用户带最近高危时间/高危24小时时段分布/高危清单一键复制脱敏列表/反馈一键标记已回应+全部批量/修复）
│   ├── report/    周报 + 月度小结（Top3/上月对比/平均强度/情绪词云）+ 连续低日提醒 + 正向鼓励条 + 本周相对最好的一天 + 本周练习足迹 + 文字版含练习足迹 + 本周平均强度 + 本周对自己说备注 + canvas 分享卡
│   ├── helper/    求助（热线/24h回访/回访关怀卡/求助后静音关怀/智能分流提示/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/一键复制求助文字发信任的人/今日关怀/英语角）
│   ├── breathe/   呼吸放松练习（4 轮引导/3 档节奏 4-4-6·4-4-4-4·6-2-6-2·结束可选 1-5 强度记录平静/呼吸徽章/累计+本周进度统计，全程本地）
│   ├── scan/      5 分钟身体扫描（10 段引导计时·完成自动记「平静」+可选 1-5 强度/累计完成次数/再来一次/每日一句轮换开场）
│   ├── station/   情绪充电站（7 张可展开心理急救卡(含拖延启动式·睡前两行清单)/一键跳呼吸·聊天·记下此刻心情/可分享）
│   ├── privacy/   隐私政策（全文）
