...
│   ├── breathe/   呼吸放松练习（4 轮引导/4 档节奏 4-4-6·4-4-4-4·6-2-6-2+发呆2分钟(累计次数陪伴计数)+自定义节奏(吸/屏/呼 2-10 秒可调·一键填入4-7-8助眠/4-4-6/6-2-6)+按时段推荐节奏(晨盒式/午放松/夜长呼)/练习中断24h内温柔回访提示·结束可选 1-5 强度记录平静(发呆模式自动隐藏强度·文案专属)/呼吸徽章/累计+本周进度统计·每日 10 分钟目标进度条+达标连击+近4周放松热力图+完成时段分布(早/中/晚/深夜)+本周vs上周次数对比·🏅最长连练纪录+最近7次放松回看·完成练习「呼吸回响」(发呆模式结束自动给专属回响·→直达心理小课为什么呼吸有用)，全程本地）
│   ├── scan/      5 分钟身体扫描（完成后可接续呼吸·按时段扫描建议·10 段引导计时·完成自动记「平静」+可选 1-5 强度+身体感受一词(8词含累了·最常完成时段统计·留最近5次回看·🫧最常反馈Top1提示)/累计完成次数/再来一次/每日一句轮换开场）
...
│   └── profile/   我的（数据足迹含年度统计(今年N条·覆盖N天)+心理小课进度条直达/成就徽章×16(点亮率百分比·🔥连学7天·已点亮自动置前并记录点亮日期·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课8节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·覆盖全部45+本地键·云端心情与徽章保留)/复制文字版「我的概览」(含结业与温故题数)+「数据都放在哪」透明说明/数据足迹(近7天格子·本月天数进度·呼吸+扫描总计)+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏分组展示(今天/更早·可一键复制全部)/按时段日签问候+累计陪伴天数(7/30/100/365天里程碑文案·可复制留念)·最近自评直达/好久没回温柔卡）
