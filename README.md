...
│   ├── helper/    求助（热线(最近拨过的自动排前·长按卡片复制号码·🕘最近拨打记录(带相对时间·累计次数·鼓励语轮换)·🫁开口前先做60秒呼吸直达)/安全包·我提前信任的人名单(支持存电话·危急一键拨打·✏️为什么信任备注·完整度提示(N人M电话)·30天未更新保鲜提醒·长按清号码·📤导出(含信任备注)/📥剪贴板恢复)/1h·24h·3d·7d四段温暖回访/走完四段一次性「毕业卡」致谢/回访关怀卡/求助后静音关怀(可提前手动结束)/智能分流提示/「现在最需要什么」三方向快速分流/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/开口求助话术生成器(9种处境·默认显示4个可展开全部→生成第一句话·可复制/✏️改一改再复制/换说法·最近复制过可再复制·含被欺负/家里吵架)+考试季家长贴士一键复制转发/🫂「帮助朋友」危机卡(怎么陪·怎么问安全·一键复制)/一键复制求助文字发信任的人/今日关怀/英语角）
...
│   └── profile/   我的（数据足迹含年度统计(今年N条·覆盖N天)+心理小课进度条直达/成就徽章×16(点亮率百分比·🔥连学7天·已点亮自动置前并记录点亮日期·🔒下一枚徽章进度提示)(🎓小学霸·学完心理小课8节)/今日心情卡(Canvas生成·可保存分享)/导出数据(云端+本地记录一并打包:小结/小课/安全包/想法盒/信匣等)/清除本机记录(二次确认·覆盖全部50+本地键·云端心情与徽章保留)/复制文字版「我的概览」(含结业与温故题数·末尾附热线提醒)+「数据都放在哪」透明说明/数据足迹(近7天格子·本月天数进度·呼吸+扫描总计)+月均强度·常用入口+本月练习计数(呼吸/扫描/写给自己)/珍藏分组展示(今天/更早·可一键复制全部)/按时段日签问候+累计陪伴天数(7/30/100/365天里程碑文案·可复制留念)·最近自评直达/好久没回温柔卡）
