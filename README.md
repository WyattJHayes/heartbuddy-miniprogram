...
│   ├── chat/        核心对话页（⭐我的常用语(已存N/5提示·满5条替换最早一条·删除二次确认·按使用次数自动置前·显示次数与最近使用时间)/夜间柔和模式(22:00-6:00 自动·进入当天轻提一句护眼·🌙手动开关优先)/快捷情绪双闭环/此刻强度滑条(7种情绪标签含🌟期待)/空输入点发送温柔托底(换引导占位)/输入超150字显示字数/倾诉后小结算/AI头像情绪态/打字草稿跨会话保留(未发送内容切走不丢)/深夜工具箱·写给自己+5分钟急救(固定预案)+睡前托一句话明早回读/📋一键复制整段对话留档(带时间戳与主题行)/重新开始前可先备份一份/回复引用/长按AI回复「换个说法」重新生成/长按任意消息存进想法盒(7天后回看)/一键总结这轮对话(总结后引导顺手去心情页记一笔)/长对话折叠·一键收起·消息跨天日期分隔线(今天/昨天/M月D日)/连续低落情绪关怀条(可顺手写给此刻的自己)/周一晨间上周陪伴小结(条数·天数·主要情绪·每周一次)/距上次聊天≥2天温和问候/高危信号首次强调认真对待/自我安抚快捷短句/夸夸我本地词库+和我做3次深呼吸即时引导/撤回我的消息/输入框按时段共鸣占位/英文角/天气/今日心语/晨间轻提醒/计划日提示/重新开始对话/长按珍藏·含撤回/今日第  N 次倾诉徽章/陪伴身份(朋友/树洞/清醒教练·本地记住)/陪伴里程碑(111·333·999句)/底栏已聊几 句·写字数计数）
...
│   ├── station/   情绪充电站（未读卡片NEW角标·按此刻心情筛选(焦虑/难过/生气/睡前/学习/平静)·12 张可展开心理急救卡(展开自动记阅读次数·📖最常翻Top1提示·阅读总数鼓励(≥5次))(含拖延启动式·睡前两行清单·发呆 2 分钟·气得发抖时 60 秒降火·5-4-3-2-1接地术·考前一夜3件事·考试当天早晨5分钟·情绪词库准确命名)/一键跳呼吸·聊天·记下此刻心情·「🎴随机抽一张」(第N次抽中缘分提示·顺带推荐另一张可点开)·卡片长按★收藏置顶(最多3张·收藏概览提示)·展开可📄复制单张·入口「心理小课」/可分享·今日一句话(换日轮换·每日可手动换一次)）
