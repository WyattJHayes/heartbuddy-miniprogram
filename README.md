...
│   ├── chat/        核心对话页（⭐我的常用语(本地存5条·点按填入长按删)/夜间柔和模式(22:00-6:00 自动深色护眼)/快捷情绪双闭环/此刻强度滑条(7种情绪标签含🌟期待)/倾诉后小结算/AI头像情绪态/打字接续草稿/深夜工具箱·写给自己+5分钟急救(固定预案)+睡前托一句话明早回读/📋一键复制整段对话留档(带时间戳与主题行)/回复引用/长按AI回复「换个说法」重新生成/一键总结这轮对话/长对话折叠·一键收起/连续低落情绪关怀条(可顺手写给此刻的自己)/周一晨间上周陪伴小结(条数·天数·主要情绪·每周一次)/高危信号首次强调认真对待/自我安抚快捷短句/夸夸我本地词库+和我做3次深呼吸即时引导/撤回我的消息/输入框按时段共鸣占位/英文角/天气/今日心语/晨间轻提醒/计划日提示/重新开始对话/长按珍藏·含撤回/今日第  N 次倾诉徽章/陪伴身份(朋友/树洞/清醒教练·本地记住)/陪伴里程碑(111·333·999句)/底栏已聊几 句·写字数计数）
...
│   ├── helper/    求助（热线/安全包·我提前信任的人名单(支持存电话·危急一键拨打)/1h·24h·3d·7d四段温暖回访/走完四段一次性「毕业卡」致谢/回访关怀卡/求助后静音关怀/智能分流提示/「现在最需要什么」三方向快速分流/呼吸·扫描快捷直达卡/「我撑过来了」5步·进度存档续走/开口求助话术生成器(7种处境→生成第一句话·可复制/换说法·含被欺负/家里吵架)+考试季家长贴士一键复制转发/一键复制求助文字发信任的人/今日关怀/英语角）
...
│   ├── edu/       心理小课（7 节心理健康小课程·本地：认识情绪/考前焦虑/睡不着/低落最小行动/朋友闹掰/社媒比较心/向大人求助不丢人·打开自动定位第一节未学(续学)·每课「随堂一问」答对自动点亮·完成度进度条·🔥连续学习天数·✏️学习心得一句话(本地)·学完7节出结业卡(结业词可复制)·每日温故一题(可再来一道)·一键重置·热线直达）
